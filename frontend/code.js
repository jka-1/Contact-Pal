const urlBase = 'http://129.212.179.109/LAMPAPI';
const extension = 'php';
...
function doRegister() {
  const firstName = document.getElementById("registerFirstName").value.trim(); // NEW
  const lastName  = document.getElementById("registerLastName").value.trim();  // NEW
  const login     = document.getElementById("registerName").value.trim();
  const password  = document.getElementById("registerPassword").value;
  const confirm   = document.getElementById("registerConfirm").value;
  const phone     = (document.getElementById("registerPhone")?.value || "").trim(); // optional
    ...
    if (this.readyState === 4) {
      try {
        ...
        if (this.status === 200 && !res.error) {
          resultEl.innerHTML = "Registration successful! You can log in now.";
          showLogin(); // redirects to index.html (already defined in register.html)
        } else {
